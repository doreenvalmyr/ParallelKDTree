...
struct DataPoint {
    ...
    int label;
    int thread_id;  // Added to keep track of the thread that owns the data point
};
...
#pragma omp parallel for
    for (int i = 0; i < data.size(); ++i) {
        data[i].thread_id = omp_get_thread_num();
    }
...
#pragma omp for
        for (int i = 0; i < data.size(); ++i) {
            if (data[i].thread_id == thread_id) {
                threadData.push_back(data[i]);
            }
        }

        // Build the KD-tree subset for the current thread
        localRoots[thread_id] = buildKDTreeSubset(threadData, 0, k);
    }
